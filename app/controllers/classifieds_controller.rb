class ClassifiedsController < ApplicationController
  ...
  def search
    ...
    @averages = Classified.where("ville":params[:ville]).group("start_date").where("number_of_guests":params[:number_of_guests]).average("price")
    @counts = Classified.where("ville":params[:ville]).group("start_date").where("number_of_guests":params[:number_of_guests]).count
  end
